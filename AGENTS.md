...
`/tdd`, `/implement`, `/unslop` for product code (`/deslop` is the same skill). `/poteto-mode` for the writing and playbook style.
`/thermo-nuclear-review` only when reviewing.
