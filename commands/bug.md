...
Read `lanes/bug.md` and `AGENTS.md`. Ask Telemetry for evidence if the ticket has none. Web app bugs: /browser-use first, then /tdd. Use /mermaid when you write a PR. Put after-state mermaid on the existing docs page. No per-PR copy. README or QUICKSTART only when the diagram is user-facing. Leave before-state on the PR.
