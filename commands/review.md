...
Read `lanes/review.md` and `AGENTS.md`. Run /code-review on the given PR only. Leave GitHub review comments. Request changes when required mermaid diagrams are missing, when an after-state belongs in docs and is missing, when a PR body is one wall of text, a line starts with #n, or a fence or list has no blank line in front of it. Do not implement. Do not merge.
