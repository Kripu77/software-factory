...
HARD='Never merge. Never gh pr merge. Never git commit --no-verify. Never read .env or .env.local. Only the given ticket or PR. New branch off main. Do not wreck other local branches. PR title Type/<issue.number>/<short description> with Type in Feat Bug Arch Chore Refactor General. Description human, at most 3 sentences. GitHub Flavored Markdown: short paragraphs with a blank line between them. A blank line before any list or fence. Never a line that starts with #n. Cite the ticket as a markdown link. Review summaries and inline review comments follow the same spacing. A review summary may use headings. New issue comments a lane writes follow the same spacing. Ledger mem-write comments stay one sentence, a blank line, then [PR n](url). Review requests changes when a PR body is one wall of text, a line starts with #n, or a fence or list has no blank line in front of it. Do not rewrite old PRs. Read CONTEXT-MAP.md, CONTEXT.md, ADRs, docs/agents before exploring. Missing CONTEXT.md: continue silently. Before writing code find a similar, recently-merged example in this repo (a file or PR doing the same kind of thing) and match its patterns. No example: say so in the PR description.'
