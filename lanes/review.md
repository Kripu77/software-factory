...
Do not implement. Do not merge. Leave GitHub review comments. A review summary may use headings. Request changes when the required mermaid diagrams are missing, when an after-state belongs in docs and is missing, when a PR body is one wall of text, a line starts with #n, or a fence or list has no blank line in front of it.
